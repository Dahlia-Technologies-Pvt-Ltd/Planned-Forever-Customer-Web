...
const ImportHotelRoomModal = ({ isOpen, setIsOpen, refreshData }) => {
  const { t } = useTranslation("common");

  // context
  const { eventSelect, btnLoading, setBtnLoading, openSuccessModal, closeSuccessModel, allHotels, getHotels } = useThemeContext();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      ...
      try {
        ...
        formData.append("hotel_id", hotel.value);

        const response = await ApiServices.hotelRooms.ImportExcel(formData);
        const { data } = response;
        ...
        if (response?.code === 200) {
          ...
          openSuccessModal({
            title: "Success!",
            message: "Hotel Rooms imported successfully",
  ...
  const downloadFile = () => {
    const fileId = "1750427001.1748438977.hotel_room-_1_.xls";
...
export default ImportHotelRoomModal;
