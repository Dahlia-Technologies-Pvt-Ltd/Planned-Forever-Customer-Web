// src/pages/Contacts/components/ArrivalTab.jsx
...
const ArrivalTab = ({ data, t }) => {
  ...
  // Table sorting
  const { items: arrivalItems, requestSort: requestArrivalSort, sortConfig: arrivalSortConfig } = useSortableData(allArrivals);

  // Table columns (excluding "Has Arrived")
  const arrivalColumns = [t("contacts.dateTime"), t("contacts.from"), t("contacts.to"), t("contacts.flightTrainNo"), t("arrival.noOfPeople")];
  ...
  // Get Arrivals for specific user
  const getArrivals = async () => {
    ...
    try {
      ...
      if (responseData.code === 200) {
        // Filter arrivals for this specific user (additional safety check)
        const userArrivals = responseData?.data?.data?.filter((arrival) => arrival.user_id === data.uuid) || [];

        setAllArrivals(userArrivals);
              ...
              {arrivalColumns?.map((head) => (
                <th
                  ...
                  onClick={() => {
                    ...
                    } else if (head === "No. of People" || head === t("arrival.noOfPeople")) {
                        ...
                        : head === "From" || head === t("contacts.from")
                          ...
                          : head === "To" || head === t("contacts.to")
                            ...
                            : head === "Flight/Train No" || head === t("contacts.flightTrainNo")
                              ? "fligh_train_no"
                              : head === "No. of People" || head === t("arrival.noOfPeople")
                                ? "no_of_person"
                                : head.toLowerCase()) && arrivalSortConfig.direction === "asc" ? (
