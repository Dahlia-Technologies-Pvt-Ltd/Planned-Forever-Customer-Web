...
const AddContact = () => {
  ...
  // Global States
  const { eventSelect, openSuccessModal, setBtnLoading, setErrorMessage, closeSuccessModel, btnLoading, eventDetail } = useThemeContext();
  ...
  const handleAllotQRCodes = async () => {
    ...
    try {
      ...
      setNewQRCodesError(err.response?.data?.message || err.message || "An error occurred during QR code allocation");
    } finally {
      setBtnLoading(false);
    }
  };
  ...
  const addNewColorCode = () => {
    ...
    ApiServices.contact
      ...
      .then((res) => {
        ...
        if (res?.status === true) {
          const newOption = { id: Date.now(), value: newColorCodeName.toLowerCase(), label: newColorCodeName };
  ...
  const handleChildAdultToggle = (childId) => {
    setChildren((prevChildren) => prevChildren.map((child) => (child.id === childId ? { ...child, isAdult: !child.isAdult } : child)));
  };
  ...
  const handleAddContactField = () => {
    setContacts((prevContacts) => [...prevContacts, { id: Date.now(), contactType: "", countryCode: "", contactNumber: "" }]);
        ...
        if (res.code === 200) {
          if (isNewChild) {
            ...
            setChildren((prevChildren) =>
              prevChildren.map((child) => (child.id === childId ? { ...child, profileImage: file, profileFile: data } : child)),
            );
  ...
  const validateForm = () => {
    ...
    //   isValid = false;
    // }
    else if (contacts.some((contact) => !contact?.contactType?.value || !contact?.countryCode?.value || !contact.contactNumber)) {
  ...
  const AddNewContact = async () => {
    ...
    let requestData = {
      ...
      contact_numbers: contacts?.map((item) => {
        return { type: item?.contactType?.value, contact_number: item?.contactNumber, country_code: item?.countryCode?.value || item?.countryCode };
      }),
          ...
          if (adultChildren && adultChildren.length > 0) {
            ...
            return Promise.all(adultContactPromises)
              ...
              .catch((err) => {
                ...
                finishContactAddition(true, "Main contact added but some adult children could not be added");
              });
          } else {
  ...
  useEffect(() => {
    if (data) {
      ...
      setSelectedColorCode(data?.color_code?.id ? { value: data?.color_code?.id, label: data?.color_code?.name } : null);
...
<div className="w-3/12">
              ...
              <div>
                <div className="mb-4 flex items-center gap-x-10">
                  ...
                  <button type="button" onClick={handleAddNewChildForm} className="flex items-center gap-2 text-green-600 hover:text-green-700">
...
<div className="w-3/12">
                ...
                {children?.map((child, index) => (
                  <div key={`child-medicine-${child.id}`} className="mt-5 space-y-4 rounded-lg border bg-blue-50 p-4">
...
<div className="w-3/12">
          ...
          <div ref={fieldRefs.contacts} className="pt-5">
            {contacts.map((contact, index) => (
              <div key={contact.id} className="mt-5 grid grid-cols-12 gap-2">
                <div className="col-span-3">
                  <Dropdown
                    ...
                    isRequired={isSubmitted && contact.contactType === "" ? true : false}
                ...
                <div className="col-span-3 mt-6">
                  <Dropdown
                    ...
                    placeholder="Country Code"
                    isRequired={isSubmitted && contact.contactType === "" ? true : false}
                    withError={isSubmitted && contact.countryCode === "" ? true : false}
                ...
                <div className="col-span-3 mt-6">
                  <Input
                    ...
                    isRequired={isSubmitted && contact.contactNumber === "" ? true : false}
                    error={isSubmitted && contact.contactNumber === "" ? "Required" : ""}
                    invisible
                  />
                </div>
                <div className="col-span-1 ml-3 mt-11">
                  {index > 0 && (
          ...
          <Input label={t("contacts.address")} placeholder="Address" textarea value={address} rows={2} onChange={(e) => setAddress(e.target.value)} />
          ...
          <div className="my-20 flex justify-end space-x-5">
            {data ? (
              <Button
              ...
              <Button icon={addLoading ? "" : <CheckIcon />} title={addLoading ? <Spinner /> : t("contacts.saveContact")} onClick={AddNewContact} />
            )}
            <Button icon={<XMarkIcon />} title="Cancel" buttonColor="bg-red-500" onClick={() => navigate(CONTACTS)} />
          </div>
        </form>
      </div>
              ...
              >
                <Dialog.Panel className="w-full max-w-xl overflow-hidden rounded-2xl bg-white p-6 shadow-xl transition-all">
                      ...
                      {qrCountsLoading ? (
                        ...
                        <>
                          <p className="text-left text-sm text-gray-600">
                            Total Available QR Codes: <span className="font-semibold">{qrCodeCounts.totalAvailable - qrCodeCounts.used}</span>
                          </p>
                          <p className="text-left text-sm text-gray-600">
                            Allocated QR Codes: <span className="font-semibold">{qrCodeCounts.allocated - qrCodeCounts.used}</span>
                          </p>
                          <p className="text-left text-sm text-gray-600">
                            Used QR Codes: <span className="font-semibold">{qrCodeCounts.used}</span>
                          </p>
                        </>
                      )}
                    </div>

                    <h2 className="sub-heading mb-1 ltr:text-left rtl:text-right">Allocate More QR Codes</h2>
                    <Input
                  ...
                  <div className="flex justify-center gap-4 pt-8">
                    <Button icon={<CheckIcon />} title="Allocate" type="button" onClick={handleAllotQRCodes} loading={btnLoading} />
                    <Button
                      icon={<XMarkIcon />}
                      title="Cancel"
                      type="button"
