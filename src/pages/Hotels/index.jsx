...
import { HOTEL_PRINT } from "../../routes/Names";
...
const Hotels = () => {
  ...
  // Table Head
  const TABLE_HEAD = [t("hotels.hotelName"), t("hotels.checkInDate"), t("hotels.checkOutDate")];

  const TABLE_HEAD_Detail = [t("hotels.roomType"), t("hotels.rooms")];
  // Context
  ...
  const detail = allHotels?.find((item) => item?.id === (activeRow || allHotels[0]?.id));

  // Table Sorting
  const { items, requestSort, sortConfig } = useSortableData(allHotels);
  ...
  // Get Hotel
  const getHotels = async (emptySearch) => {
    try {
      setLoading(true);
  ...
  const handleDeleteHotel = async () => {
    try {
      ...
      const res = await ApiServices.hotels.deleteHotel(openDeleteModal?.data?.id);
      const { data, message } = res;
      ...
      if (data.code === 200) {
        ...
        openSuccessModal({
          ...
          message: t("hotels.hotelDeleteSuccess"),
          onClickDone: (close) => {
            closeSuccessModel();
          },
        });
  ...
  return (
    <>
                        ...
                        <th
                          ...
                          onClick={() => {
                            ...
                            if (head === "Hotel Name") {
                              ...
                              sortKey = "room_no";
                            } else if (head === "Check-in Date") {
                              sortKey = "check_in";
                            } else if (head === "Checkout Date") {
                              sortKey = "check_out";
                            } else {
                      ...
                      items.map((item, index) => (
                        ...
                        >
                          ...
                          {/* <td className="py-3 pr-3 pl-4 3xl:px-4">
                            <p className="text-xs font-normal text-primary-color-200 3xl:text-sm">{item?.room_type || "-"}</p>
                          </td>

                          <td className="py-3 pr-3 pl-4 3xl:px-4">
                            <p className="text-xs font-normal text-primary-color-200 3xl:text-sm">
                              <p className="text-xs font-normal text-primary-color-200 3xl:text-sm">{item?.room_no || "-"}</p>
                            </p>
                          </td> */}
                ...
                <div className="-mr-6 h-[70vh] space-y-8 overflow-y-auto 3xl:mr-0">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-5 3xl:grid-cols-3">
                      <TitleValue title={t("hotels.hotelName")} value={detail?.name || "-"} />
                      {/* <TitleValue title="Room Type" value={detail?.room_type || "-"} />
                      <TitleValue title="Room No" value={detail?.room_no || "-"} /> */}
                      <TitleValue title={t("hotels.checkInDate")} value={moment.unix(detail?.check_in).format("D MMM YYYY") || "-"} />
                      <TitleValue title={t("hotels.checkOutDate")} value={moment.unix(detail?.check_out).format("D MMM YYYY") || "-"} />
                    </div>
                  </div>
                  <TitleValue title={t("hotels.hotelAddress")} value={detail?.address || "-"} />

                  {/* <div className="">
                    <h3 className="text-xs text-info-color">{t("hotels.room")}</h3>
                    <div className="mb-8 overflow-x-auto">
                      <table className="w-full text-left">
                        ...
                        <tbody>
                          ...
                          ) : detail?.room?.length > 0 ? (
                            detail?.room?.map((item, index) => (
                              <tr key={index} className="cursor-pointer">
                                <td className="py-3 pr-4">
                                  <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">{item?.type || "-"}</p>
                                </td>
                                <td className="py-3 pr-4">
                                  <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">{item?.no_of_rooms || "-"}</p>
                                </td>
                              </tr>
                            ))
                          ) : (
      ...
      <ConfirmationModal
        ...
        message={t("hotels.hotelDeleteConf")}
