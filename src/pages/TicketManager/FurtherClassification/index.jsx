...
import AddFurtherClassification from "./AddFurtherClassification";
...
const FurtherClassification = () => {
  ...
  // Table Head
  const TABLE_HEAD = [
    t("classification.categoryName"),
    t("classification.subCategoryName"),
    t("classification.furtherClassification"),
    t("classification.customField"),
    t("headings.actions"),
  ];
  ...
  // Data
  const [allFurtherClassification, setAllFurtherClassification] = useState([]);
  ...
  // Table Sorting
  const { items, requestSort, sortConfig } = useSortableData(allFurtherClassification);
  ...
  const getFurtherClassification = async (emptySearch) => {
    let payload = {
    ...
    const res = await ApiServices.further_classification
      .getFurtherClassification(payload)
      .then((res) => {
        const { data, message } = res;

        if (data?.code === 200) {
          setLoading(false);
          setAllFurtherClassification(data.data.data);
  ...
  // Delete Further Classifications

  const handleDeleteClassification = async () => {
    try {
      setBtnLoading(true);

      const response = await ApiServices.further_classification.deleteFurtherClassification(openDeleteModal?.data?.id);
      if (response.data.code === 200) {
        setBtnLoading(false);
        getFurtherClassification();
        setOpenDeleteModal({ open: false, data: null });
        openSuccessModal({
          title: t("messages.success"),
          message: t("classification.furtherClassificationDeleteSuccess"),
          onClickDone: (close) => {
            closeSuccessModel();
          },
        });
  ...
  const handleKeyPress = async (e) => {
    if (e.key === "Enter") {
      if (searchText.trim() !== "") {
        await getFurtherClassification(false);
        setCurrentPage(1);
      }
    }
  };
  ...
  return (
    <>
        ...
        <div className="col-span-12 lg:col-span-12">
          <div className="card min-h-[82vh]">
            <div className="flex justify-between">
              <h3 className="heading">Further Classifications</h3>
              <div className="flex w-full items-center justify-between">
                <Button title={t("classification.addFurtherClassification")} onClick={() => setOpenAddNewModal({ open: true, data: null })} />
                      ...
                      items.map((item, index) => (
                        <tr key={item?.uuid} className="even:bg-gray-50">
                          <td className="py-3 pl-6 pr-4">
                            <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">{item?.sub_category?.category?.name}</p>
                          </td>
      ...
      <ConfirmationModal
        ...
        handleSubmit={handleDeleteClassification}
        message={t("classification.furtherClassificationDeleteConf")}
