...
import AddNearbyAttractionModal from "./AddNearbyAttractionModal";
...
const Venues = () => {
  ...
  // Table Head
  const TABLE_HEAD = [t("nearByAttraction.attractionName"), t("nearByAttraction.contactPerson"), t("nearByAttraction.phone")];
  ...
  const getVenues = async (emptySearch) => {
    try {
      ...
      const res = await ApiServices.Nearby_Attraction.getNearbyAttractionList(payload);
  ...
  const handleDeleteVenue = async () => {
    try {
      ...
      const res = await ApiServices.Nearby_Attraction.deleteNearbyAttraction(openDeleteModal?.data?.id);
      const { data, message } = res;
      ...
      if (data.code === 200) {
        ...
        openSuccessModal({
          ...
          message: t("nearByAttraction.nearByAttractionDeleteSuccess"),
          onClickDone: () => {
            closeSuccessModel();
          },
        });
  ...
  return (
    <>
        ...
        <div className="col-span-12 lg:col-span-7">
          <div className="card min-h-[82vh]">
            <div className="flex justify-between">
              <h3 className="heading">Venues</h3>
                        ...
                        >
                          ...
                          <td className="py-3 pl-4 pr-3 3xl:px-4">
                            <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">
                              {item?.contact_person_contacts[0] && (
                                <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">{item?.contact_person_contacts[0].phone}</p>
                              )}
                            </p>
                          </td>
                ...
                <div className="-mr-6 h-[70vh] space-y-8 overflow-y-auto 3xl:mr-0">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-5 3xl:grid-cols-2">
                      <TitleValue title="Designation" value={detail?.contact_person_designation || "-"} />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {detail?.contact_person_contacts?.map((contact, index) => (
                      <p key={index} className="mt-5 flex items-center gap-x-5 space-y-0">
                        <TitleValue title={t("nearByAttraction.contactType")} value={contact.type || "-"} />
                        <TitleValue title={t("nearByAttraction.contactNumber")} value={contact?.phone || "-"} />
                        <TitleValue title={t("nearByAttraction.countryCode")} value={contact?.countryCode || "-"} />
                      </p>
                    ))}
                  </div>

                  <TitleValue title={t("nearByAttraction.address")} value={detail?.address || "-"} />
                      ...
                      <div className="w-full">
                        ...
                        <div className="grid w-full grid-cols-3 gap-2">
                          {detail?.images?.map((item) => (
                            <img src={mediaUrl + item} alt="image" className="h-full w-full rounded-10 object-cover" />
                          ))}
                        </div>
                      </div>
                    )}
                  ...
                  <div>
                    ...
                    <TitleValue title={t("nearByAttraction.description")} value={detail?.description || "-"} />
                  </div>
                </div>
              </>
            ) : (
      ...
      <ConfirmationModal
        ...
        message={t("nearByAttraction.nearByAttractionDeleteConf")}
