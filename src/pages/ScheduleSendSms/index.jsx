...
import ScheduleSendSmsModal from "./ScheduleSendSmsModal";
...
// local data
const CarAllocationData = [
  {
    title: "Sangeet Ceremony",
    allocated_to_Date: "03-05-2023",
    allocated_to_Time: "10:30 AM",
    jobStatus: "Software Engineer",
  },
];

const SchedlueSendSms = () => {
  ...
  const getScheduleList = (emptySearch) => {
    ...
    ApiServices.ScheduledSMS.getScheduleSmsList(payload)
      .then((res) => {
  ...
  const handleDeleteScheduleSms = async () => {
    try {
      ...
      const res = await ApiServices.ScheduledSMS.deleteScheduleSms(openDeleteModal?.data?.id);
      const { data, message } = res;
      ...
      if (data.code === 200) {
        ...
        openSuccessModal({
          ...
          message: t("scheduleSendSms.sendSMSDeleteSuccess"),
          onClickDone: (close) => {
            closeSuccessModel();
          },
        });
  ...
  return (
    <>
                        ...
                        >
                          ...
                          <td className="py-3 pl-4 pr-3 3xl:px-4">
                            <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">
                              {moment.unix(item?.send_at).format("D MMM YYYY  h:mm A") || "-"}
                            </p>
                          </td>
                ...
                <div className="-mr-6 h-[70vh] space-y-8 overflow-y-auto 3xl:mr-0">
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-5 3xl:grid-cols-3">
                      <TitleValue title={t("scheduleSendSms.scheduleTitle")} value={detail?.title || "-"} />
                      <TitleValue
                        title={t("scheduleSendSms.scheduleOn")}
                        value={moment.unix(detail?.send_at).format("D MMM YYYY  h:mm A") || "-"}
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <TitleValue title={t("scheduleSendSms.message")} value={detail?.message || "-"} />
                  </div>
                </div>
              </>
            ) : (
      ...
      <ConfirmationModal
        ...
        message={t("scheduleSendSms.scheduleDeleteConf")}
...
export default SchedlueSendSms;
