...
import { EVENT_PRINT } from "../../routes/Names";
...
// Table Head
const TABLE_HEAD = ["Event Name", "Venue", "Start Date & Time", "End Date & Time"];
...
const Events = () => {
  ...
  const detail = allEvents?.find((item) => item?.id === (activeRow || allEvents[0]?.id));

  // Table Sorting
  const { items, requestSort, sortConfig } = useSortableData(allEvents);
  ...
  // Get Events
  const getEvents = async (emptySearch) => {
    try {
      setLoading(true);
      ...
      const res = await ApiServices?.events?.getEvents(payload);
  ...
  const handleDeleteEvent = async () => {
    try {
      ...
      const res = await ApiServices.events.deleteEvent(openDeleteModal?.data?.id);
      const { data, message } = res;
            ...
            <div className="flex justify-between">
              ...
              <div className="flex w-full items-center justify-between">
                <div className="flex items-center gap-x-3">
                  <Button title="Add Event" onClick={() => setAddNewModal(true)} />
                  <Link to={EVENT_PRINT}>
                      ...
                      items?.map((item, index) => (
                        ...
                        >
                          ...
                          <td className="py-3 pl-4 3xl:px-4">
                            <p className="text-primary-color-200 text-xs font-normal 3xl:text-sm">{item?.venue?.name}</p>
                          </td>
                ...
                <div className="h-[70vh] overflow-y-auto">
                  <div className="mt-6">
                    ...
                    <div className="flex flex-wrap justify-between gap-5">
                      <TitleValue title="Event Name" value={detail?.name || "-"} />
                      <TitleValue title="Venue" value={detail?.venue?.name || "-"} />
                      <TitleValue title="Start Date & Time" value={getLocalDateFromUnixTimestamp(detail?.start_date, "DD MMM, YYYY, HH:mmA")} />
                      <TitleValue title="End Date & Time" value={getLocalDateFromUnixTimestamp(detail?.end_date, "DD MMM, YYYY, HH:mmA")} />
                    </div>
                  </div>
