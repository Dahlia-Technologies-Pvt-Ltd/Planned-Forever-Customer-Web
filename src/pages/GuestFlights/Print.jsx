...
import { GUEST_FLIGHTS } from "../../routes/Names";
...
const GuestFlightsPrint = () => {
  const { t } = useTranslation("common");

  const TABLE_HEAD_GUEST_FLIGHTS = [
    t("guestFlight.guestName"),
    t("guestFlight.departureDate"),
    t("guestFlight.departureTime"),
    t("guestFlight.arrivalDate"),
    t("guestFlight.arrivalTime"),
  ...
  const [allGuestFlightsList, setAllGuestFlightsList] = useState([]);
  ...
  // get guest flights list to print
  const getGuestFlightsListToPrint = async () => {
    try {
      let payload;
      if (updatedReport) {
        payload = {
      ...
      setLoading(true);
      const result = await ApiServices.guestFlight.getGuestFlightReport(payload);
      if (result.data.code === 200) {
        setAllGuestFlightsList(result?.data?.data?.data);
        setLoading(false);
      }
    } catch (error) {
      console.error("Error fetching guest flights report data:", error);
      setLoading(false);
    }
  };
  ...
  const PrintableGuestFlightsList = React.forwardRef(({ allGuestFlightsList, loading }, ref) => (
    <div ref={ref} className="printableGuestFlightsList pr-2">
      <table className="w-full text-left">
        <thead>
          <tr>
            {TABLE_HEAD_GUEST_FLIGHTS.map((head) => (
        ...
        <tbody>
          ...
          ) : allGuestFlightsList && allGuestFlightsList.length > 0 ? (
            allGuestFlightsList.map((item, index) => (
              <tr key={item?.id}>
  ...
  return (
    <>
      {(userData?.role === "superAdmin" || userData?.role?.permissions?.includes("GuestFlights")) && (
        <div onClick={() => navigate(GUEST_FLIGHTS)} className={`mb-5 flex cursor-pointer text-base font-medium text-secondary hover:underline`}>
          <ArrowLeftIcon className="mr-2 h-6 w-4 text-secondary" />
          <span>Back to Guest Flights list</span>
        </div>
      )}
      <div className="card min-h-[76vh]">
        <h3 className="heading">Print Guest Flights</h3>
        <div className="grid grid-cols-12 gap-x-5">
          <div className="col-span-6 mb-4 2xl:col-span-3">
            <h4 className="mb-2 text-base font-semibold text-gray-700">{t("guestFlight.departureDate")}</h4>
          ...
          <div className="col-span-6 mb-4 2xl:col-span-3">
            <h4 className="mb-2 text-base font-semibold text-gray-700">{t("guestFlight.arrivalDate")}</h4>
            <div className="grid grid-cols-12 gap-4 xl:grid-cols-12">
              <div className="col-span-6">
                <Input
                  type="date"
          ...
          <div className="col-span-8 mb-6 mt-8 2xl:col-span-6">
            <div className="grid grid-cols-12 gap-4 xl:grid-cols-12">
              <div className="col-span-4">
                <Dropdown
                  ...
                  title={t("guestFlight.city")}
                />
              </div>
              <div className="col-span-8 flex items-end gap-4">
                <Button title={t("guestFlight.updateReport")} onClick={handleFilter} />
                <Button title={t("buttons.clearFilters") || "Clear Filters"} onClick={clearFilters} variant="outline" />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-5 h-[48vh] overflow-y-auto overflow-x-hidden">
          <div className="-mx-6 mb-8 overflow-x-auto">
            <PrintableGuestFlightsList ref={componentRef} allGuestFlightsList={allGuestFlightsList} loading={loading} />
          </div>
        </div>
...
export default GuestFlightsPrint;
