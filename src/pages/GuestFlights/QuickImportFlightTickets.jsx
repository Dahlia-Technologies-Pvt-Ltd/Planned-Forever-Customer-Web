...
const QuickImportFlightTickets = ({ openQuickImport, setOpenQuickImport, refreshData, onSend }) => {
  ...
  const [deparaturedateTime, setDeparaturedateTime] = useState("");
  ...
  const sendToParent = () => {
    onSend(arrivingFrom, arrivingAt, deparaturedateTime, arrivalFlightTrainNo,arrivalDateAndTime); // 👈 separate values
    handleClose();
  };
      ...
      if (["jpg", "jpeg", "png"].includes(ext)) {
        ...
        if (result?.data?.text) {
          ...
          if (date) {
            ...
            if (m.isValid()) {
              setDeparaturedateTime(m.format("YYYY-MM-DD HH:mm"));
            } else {
              console.error("Invalid date:", dateTimeStr);
            }
          }
          const arrivalDate = pdfData?.[0]?.arrivalDate?.trim();
          const arrivalTime = pdfData?.[0]?.arrivalTime?.trim() || "00:00";
          if (arrivalDate) {
            const dateTimeStr = `${arrivalDate} ${arrivalTime}`;
            const m = moment(dateTimeStr, "DD MMM YYYY HH:mm", true);
            if (m.isValid()) {
...
export default QuickImportFlightTickets;
