...
const ImportGuestFlightModal = ({ isOpen, setIsOpen, refreshData }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isValid) {
      ...
      try {
        ...
        const response = await ApiServices.guestFlight.ImportExcel(formData);
        const { data } = response;
        ...
        if (response?.code === 200) {
          ...
          openSuccessModal({
            title: "Success!",
            message: "Guest Flights imported successfully",
  ...
  const downloadFile = () => {
    const fileId = "1756239929.guest-flight-train-import.xlsx";
                ...
                <Dialog.Panel className="w-full max-w-3xl rounded-2xl bg-white p-8 shadow-xl transition-all">
                  <div className="mb-5 flex items-center justify-between">
                    <Dialog.Title as="h3" className="font-poppins text-lg font-semibold leading-7 text-secondary-color">
                      Import Guest Flight Excel File
                    </Dialog.Title>
...
export default ImportGuestFlightModal;
